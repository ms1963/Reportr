...
current, high-rated research papers, articles, blog-posts. It concurrrently downloads thep up to a maximum number
of files, rates and summarized them. The research results are then shown on the screen and stored in an .md document.
It is possible to configure Reportr to run using time schedules like "all two week on Monday at 8:00". The documents
are also partitioned in chunks and stored in a vector database. So far Reportr is not using this database, but
it can be easily extended with a query component that allows to ask questions about the stored content.
